...
Five constraints, each learned the hard way. Do not undo them without re-running the spike:
...
- **The glyph run's `FontSize` is scaled down to match the body font's line box** (`ChecklistHelper.NormaliseGlyphLineBox`). WPF's default `MaxHeight` line stacking sizes a line to its tallest run, and `Segoe UI Symbol` has `LineSpacing` 1.3301 versus 1.1621 for Cascadia Mono and 1.1362 for Georgia — an unscaled glyph therefore inflates every checklist line by ~15%, which reads as 1.5 spacing next to plain lines. The scale factor is `min(1, bodyLineSpacing / glyphLineSpacing)`; `FontSize` survives the RTF round-trip, which is why the fix is a font size and not `Paragraph.LineHeight` (a layout property RTF drops). `ChecklistHelper.NormaliseDocument` re-pins every checklist item after each RTF load (`LoadRtfContent`) so notes saved by older builds repair their spacing on open. Do not remove the scaling, and do not remove the load-time pass.
